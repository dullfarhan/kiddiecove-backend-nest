...
export class PostService {
  ...
  async getCurrentUser(req, userType) {
    try {
      let currentUser;
  ...
  async getAllPostsForParent(req: Request, res: Response) {
    ...
    const response = await this.getCurrentUser(req, UserType.PARENT);
    if (response.status === Constant.FAIL)
      return Util.getBadRequest(response.message, res);
  ...
  async getAllPostsForTeacher(req, res) {
    ...
    const response = await this.getCurrentUser(req, UserType.TEACHER);
    if (response.status === Constant.FAIL)
      return Util.getBadRequest(response.message, res);
